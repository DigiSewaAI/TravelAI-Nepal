...
Route::get('/robots.txt', function () {
    $lines = [
        ...
        'Disallow: /booking/qr/',
        '',
        '# ===== Booking (user-specific) =====',
        'Disallow: /explore/service/*/book',
        'Disallow: /service/confirmation/',
        '',
        '# ===== API & internal tools =====',
        'Disallow: /api/',
        'Disallow: /telescope/',
...
// =======================================
// 9. QR CODE & CHECK-IN ROUTES
// =======================================
// QR Code image generation
Route::get('/booking/qr/{booking}', function ($id) {
    $booking = Booking::findOrFail($id);
    $qrSecurityService = app(QrSecurityService::class);
    $secureUrl = $qrSecurityService->generateSecureQrUrl($booking);
    return response(QrCode::size(300)->generate($secureUrl))
...
Route::get('/safety/search', [App\Http\Controllers\SafetyController::class, 'searchDestination'])
    ->name('safety.search');
