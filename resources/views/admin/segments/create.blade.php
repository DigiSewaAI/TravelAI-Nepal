...
@section('header', 'Create Segment')
@section('content')
<form method="POST" action="{{ route('admin.segments.store') }}" class="bg-white p-6 rounded-lg shadow">
    @csrf
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="number" name="sequence" class="w-full border rounded-lg px-4 py-2" value="{{ old('sequence', 0) }}" min="0" required>
        </div>
    </div>
    <div class="grid grid-cols-2 gap-4 mt-4">
        <div>
            ...
            <select name="from_waypoint_id" class="w-full border rounded-lg px-4 py-2" required>
                ...
                <option value="{{ $id }}" {{ old('from_waypoint_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="to_waypoint_id" class="w-full border rounded-lg px-4 py-2" required>
                @foreach($waypoints as $id => $name)
                <option value="{{ $id }}" {{ old('to_waypoint_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="grid grid-cols-3 gap-4 mt-4">
        <div>
            ...
            <input type="number" step="0.01" name="distance_km" class="w-full border rounded-lg px-4 py-2" value="{{ old('distance_km') }}" required>
        </div>
        <div>
            <label class="block font-medium mb-1">Est. Time (hours)</label>
            <input type="number" step="0.1" name="estimated_time_hours" class="w-full border rounded-lg px-4 py-2" value="{{ old('estimated_time_hours') }}">
        </div>
        <div>
            <label class="block font-medium mb-1">Elevation Gain (m)</label>
            <input type="number" name="elevation_gain_m" class="w-full border rounded-lg px-4 py-2" value="{{ old('elevation_gain_m') }}" min="0">
        </div>
    </div>
    <div class="mt-4">
        <label class="block font-medium mb-1">Elevation Loss (m)</label>
        <input type="number" name="elevation_loss_m" class="w-full border rounded-lg px-4 py-2" value="{{ old('elevation_loss_m') }}" min="0">
    </div>
    <div class="mt-6">
