...
@section('title', 'Edit User | TravelAI Nepal')
...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border p-6">
    <form method="POST" action="{{ route('admin.users.update', $user) }}">
        ...
        <div class="space-y-4">
            ...
            <div>
                <label class="block text-gray-700 font-semibold mb-1">Password (leave blank to keep current)</label>
            ...
            <div>
                ...
                <select name="role" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="traveler" {{ old('role', $user->role) === 'traveler' ? 'selected' : '' }}>Traveler</option>
                    <option value="provider_owner" {{ old('role', $user->role) === 'provider_owner' ? 'selected' : '' }}>Provider Owner</option>
