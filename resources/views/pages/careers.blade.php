...
<div class="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-16 px-4 text-center">
    <h1 class="text-4xl md:text-5xl font-bold">{{ __('messages.careers_hero_title') }}</h1>
...
<div class="max-w-5xl mx-auto px-4 py-12">
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-12">
        <div class="bg-white p-6 rounded-xl shadow-md border text-center hover:shadow-lg transition">
            ...
            <h3 class="font-bold text-gray-800 mt-2">{{ __('messages.careers_why_innovative') }}</h3>
            <p class="text-sm text-gray-500">{{ __('messages.careers_why_innovative_desc') }}</p>
        </div>
        <div class="bg-white p-6 rounded-xl shadow-md border text-center hover:shadow-lg transition">
            <i class="fas fa-globe-asia text-blue-600 text-3xl"></i>
            <h3 class="font-bold text-gray-800 mt-2">{{ __('messages.careers_why_global') }}</h3>
            <p class="text-sm text-gray-500">{{ __('messages.careers_why_global_desc') }}</p>
        </div>
        <div class="bg-white p-6 rounded-xl shadow-md border text-center hover:shadow-lg transition">
            <i class="fas fa-users text-blue-600 text-3xl"></i>
            <h3 class="font-bold text-gray-800 mt-2">{{ __('messages.careers_why_team') }}</h3>
            <p class="text-sm text-gray-500">{{ __('messages.careers_why_team_desc') }}</p>
        </div>
    </div>

    {{-- ========== JOB LISTINGS ========== --}}
    <h2 class="text-2xl font-bold text-gray-900 mb-6">{{ __('messages.careers_open_positions') }}</h2>
    <p class="text-gray-500 mb-8">{{ __('messages.careers_open_positions_sub') }}</p>
    ...
    <div class="space-y-6">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-wrap gap-2 mt-1">
                            ...
                            <span class="text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full">
                                <i class="fas fa-briefcase mr-1"></i>{{ $job['type'] ?? __('messages.careers_full_time') }}
                            </span>
                            <span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">
                                <i class="fas fa-circle text-[6px] mr-1 align-middle"></i> {{ __('messages.careers_active') }}
                            </span>
                        </div>
                    </div>
                    <span class="text-xs text-gray-400 whitespace-nowrap">
                        <i class="far fa-clock mr-1"></i> {{ __('messages.careers_posted') }} {{ $job['posted_at'] ?? __('messages.careers_recently') }}
                    </span>
                </div>
        ...
        @empty
            <div class="text-center py-12 bg-white rounded-xl border">
                ...
                <h3 class="text-xl font-semibold text-gray-600">{{ __('messages.careers_no_positions') }}</h3>
                <p class="text-gray-400 mt-2">{{ __('messages.careers_no_positions_sub') }}</p>
            </div>
        @endforelse
    </div>
